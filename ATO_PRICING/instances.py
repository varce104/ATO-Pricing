...
def instances(size, bom, costs, price_param, demand, lead_times, show):
    ...
    if inst == 1:
        ...
        seeds = [seed + i for i in range(inst)]
        results = []
        files = []
        ...
        for seed in seeds:
            ...
            results.append(res)
            files.append(f"var_data/MS_SL_inst{seed}.xlsx")
            
        df = pd.DataFrame(results)
        df.to_csv(f"var_results/mean_res_{range(inst)}_insts.csv", index=True)
        if show_var:
            average_excel_solutions(files, output_path="var_results/mean_vars_by_inst/avg_sol.xlsx")